...
class Compilation {
    ...
    //BFS构建模块队列
    // _buildModuleQueueJob(entry) {

    //     const queue = this.buildQueue
    //     //1 通过入口文件构建文件资源
    //     const mainModule = this.createModule(entry)
    ...
    // 生成所有模块的JS代码
    _generateModulesCodeJob() {
        for (let module of this.buildQueue) {
...
const process = {env:{NODE_ENV:'${mode}'}};
    ...
    createManifestPart(p) {
        //构建文件资源
        const module = this.createModule(p)
        const dirname = path.dirname(module?.filePath) // 获取当前处理文件的绝对路径
        module.mapping = {} // 文件的依赖map

        for (const relativePath of module.dependencies) {// 遍历文件依赖的文件
    ...
    //! 替换需要的manifest模块(热更新)
    updateManifest(modulesQueue) {
        modulesQueue.forEach(module => {
            const target = this.manifest.find((part) => module.filePath === part.filePath)
            target.sourceCode = module.sourceCode
            target.dependencies = module.dependencies
            target.mapping = module.mapping
        })
    }
    ...
    getNeedUpdateFiles(path) {
        var importors = [path]
        this.manifest.forEach(module => {
            Object.keys(module.mapping).forEach(p => {
                if (module.mapping[p] === path) {
                    importors.push(module.filePath)
                }
            })
        })
    ...
    createNewBundle(path) {
        ...
        const modulesQueue = needUpdateFiles.map(p => this.createManifestPart(p))
        this.updateManifest(modulesQueue)
    ...
    // 最终构建产物, 此时已经完成了module的添加和chunk工作
    // 在这里进行JS代码生成
    seal() { }
